...
import os, sys, re, collections, csv, random, glob, time, itertools
	...
	########## AJAX ##########
		...
		try:
			...
			if request.form['media'] == 'mobile': # if mobile, show fewer results
				max_len = 300 # max results for string search
			else:
				max_len = 500
			...
			if request.form['mode'] == 'word':
				...
				### ONE WORD ###
				if request.form['input_word2'].strip() == '': # only one input
					query = request.form['input_word1']
					# ngrams = concatenated and sorted list [[left_bigram, count, right_bigram, count],...] max 30 lines
				...
				### TWO WORDS ###
					...
					query1 = request.form['input_word1']
					query2 = request.form['input_word2']
			...
			########## TOP PAGE -> SEARCH BY STRING ##########
				...
				### ONE STRING ###
				if request.form['input_string2'].strip() == '': # only one input
					query = request.form['input_string1']
				...
				### TWO STRINGS ###
					...
					query1 = request.form['input_string1']
					query2 = request.form['input_string2']
			...
			########## RESULT OF SEARCH BY WORD -> SEARCH BY STRING ##########
				...
				query = request.form['input_word2string']
...
def page_true():
	if request.method == 'GET':
		...
		df = pd.read_json(f'static/truedrama/{drama}.json')
